...
static void updateM4aFileDurations(FILE *fp, int freq, xldoffset_t total)
{
	...
	if(fread(&tmp,4,1,fp) < 1) goto end;
	timescale_moov = OSSwapBigToHostInt32(tmp);
...
static int updateM4aFileInfo(FILE *fp)
{
	...
	frequency = OSSwapBigToHostInt32(freq);
	...
	if(frequency <= 65535) {
		...
		*/
		freq = frequency << 16;
		freq = OSSwapHostToBigInt32(freq);
		if(fseeko(fp,4,SEEK_CUR) != 0) goto end;
	...
	n = OSSwapBigToHostInt32(n);
	m=0;
	for(i=0;i<n;i++) {
		if(fread(&tmp,4,1,fp) < 1) goto end;
		if(fread(&tmp2,4,1,fp) < 1) goto end;
		tmp = OSSwapBigToHostInt32(tmp);
		tmp2 = OSSwapBigToHostInt32(tmp2);
	...
	if(fseeko(fp,8,SEEK_CUR) != 0) goto end;
	if(fread(&tmp,4,1,fp) < 1) goto end;
	totalALACFrames = OSSwapBigToHostInt32(tmp);
	...
	for(i=0;i<totalALACFrames;i++) {
		if(fread(&tmp,4,1,fp) < 1) goto end;
		ALACFrameSizeTable[i] = OSSwapBigToHostInt32(tmp);
	}
	...
	for(i=0;i<n;i++) {
		if(fread(&tmp,4,1,fp) < 1) goto end;
		chunkIndexTable[i] = OSSwapBigToHostInt32(tmp);
	}
...
static void updateUdta(FILE *fp, NSMutableData *udta)
{
	...
	while(current < len) {
		...
		atomLength = OSSwapBigToHostInt32(atomLength);
		...
		if(!memcmp("----",atom,4)) {
			...
			if([nameStr isEqualToString:@"iTunNORM"]) {
				...
				tmp = OSSwapBigToHostInt32(tmp);
				flag = OSSwapBigToHostInt32(flag);
				if(flag != 1) goto last;
				if(tmp <= 16) goto last;
...
- (void)optimizeAtoms
{
	...
	if(floor(NSAppKitVersionNumber) > NSAppKitVersionNumber10_4 && floor(NSAppKitVersionNumber) < 1343) {
		if(![[[NSBundle bundleWithIdentifier:@"jp.tmkk.XLDAlacOutput"] objectForInfoDictionaryKey: @"XLDAlacOutputUseDiskCache"] boolValue])
	...
	while(1) { //skip until trak;
		...
		if(!memcmp(atom,"trak",4)) {
			...
			if(trackID == 1) { // audio track; increase length by 20 bytes because tref is appended
				...
				updatedLength = OSSwapHostToBigInt32(updatedLength);
