		...
		if(result->parent) {
			...
			if(currentTrack <= obj->trackNumber) {
				...
				if(currentFrame+count >= obj->actualLengthArr[currentTrack-1]) {
					double trackGain, peak;
					XLDGainAnalyzer_CommitSamples(result->analyzer, buffer, obj->actualLengthArr[currentTrack-1]-currentFrame);
					XLDGainAnalyzer_GetCurrentTrackGainAndPeak(result->analyzer, &trackGain, &peak);
					targetResult->trackGain = trackGain;
					targetResult->peak = peak;
					XLDGainAnalyzer_CommitSamples(result->analyzer, buffer+2*(obj->actualLengthArr[currentTrack-1]-currentFrame), count+currentFrame-obj->actualLengthArr[currentTrack-1]);
					//NSLog(@"track %d: gain %.2f, peak %f",currentTrack,targetResult->trackGain,targetResult->peak);
				}
				else XLDGainAnalyzer_CommitSamples(result->analyzer, buffer, count);
			}
		}
		else XLDGainAnalyzer_CommitSamples(result->analyzer, buffer, count);
	}
